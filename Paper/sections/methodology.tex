...
\begin{equation}\label{eq:polynomial-size-profile}
  ...
  + \varepsilon_{ist}.
\end{equation}
This specification is motivated by the size-wage literature, where firm-size
effects can be represented either by size intervals or by functions of log firm
size \citep{AbowdKramarzMargolis1999}. In our setting, the quadratic
approximation asks whether the categorical estimates are consistent with a
smooth, concave size-wage profile. It is not a more precise measure of firm size:
it relies on the same assigned values for grouped and top-coded firm-size
categories.
