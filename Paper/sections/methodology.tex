...
We also estimate a quadratic approximation to the size-wage profile:
\begin{equation}\label{eq:polynomial-size-profile}
  \ln(w_{ist}) =
  \alpha
  + \rho_1 \ln(\widetilde{Size}_{ist})
  + \rho_2 [\ln(\widetilde{Size}_{ist})]^2
  + X_{ist}'\gamma
  + \lambda_{s \times t}
  + \varepsilon_{ist}.
\end{equation}
This specification is motivated by the econometric literature on ordinal
qualitative regressors, which asks how much structure can be imposed on ordered
categories relative to a conventional dummy-variable specification
\citep{Terza1987}. In our setting, the quadratic approximation asks whether the
sequence of categorical estimates can be summarized by a smooth size-wage
profile. It is not a more precise measure of firm size: it relies on the same
assigned values for grouped and top-coded firm-size categories. The categorical
specifications therefore remain the main estimates, and the polynomial
specification is used as a robustness and shape check.
