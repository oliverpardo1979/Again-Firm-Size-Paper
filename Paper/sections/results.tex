...
Appendix Table~\ref{tab:firm-size-local-geography-robustness} shows that this result is not driven by broad geographic differences across departments. The 101+ coefficient remains positive and statistically significant after adding department-year fixed effects, after absorbing sector-department-year fixed effects, and after adding occupational-position controls to that local-geography specification. Appendix Table~\ref{tab:firm-size-local-geography-robustness-101-reference} reports the same robustness exercise using 101+ workers as the omitted category.
