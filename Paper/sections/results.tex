...
The minimum-wage evidence helps interpret why the formal-worker profile is so flat relative to formal solo workers. Figure~\ref{fig:minwage-distribution-formality-size} shows visible bunching around the statutory hourly minimum among formal workers in intermediate-size firms. Appendix Table~\ref{tab:minimum-wage-monthly-bunching-2025} gives the same comparison in monthly terms: in 2025, between 39.1 and 46.3 percent of formal workers in firms with 4--100 workers had implied monthly labor income between 0.9 and 1.1 monthly minimum wages, compared with 17.8 percent among formal solo workers and 29.2 percent among formal workers in 101+ firms. This concentration around the minimum wage is consistent with wage compression in the formal categories where the adjusted firm-size premium is weakest. It does not prove that the minimum wage causes the flat formal profile, but it identifies a wage-setting benchmark that is much more visible in intermediate formal firms than in the two formal extremes.
...
Tables~\ref{tab:firm-size-local-geography-formality-robustness}--\ref{tab:firm-size-local-geography-formality-robustness-11-19-reference} report the same within-formality comparisons under more demanding controls. The specification keeps the firm-size-by-formality interaction and progressively adds sector-year fixed effects, department-year fixed effects, sector-department-year fixed effects, and, in the final column, occupational-position controls. The three tables differ only in the omitted firm-size category: solo workers, workers in 101+ firms, and workers in firms with 11--19 workers. Each coefficient compares workers within the same formality status, so the informal panel should be read as comparisons among informal workers and the formal panel as comparisons among formal workers.
...
Column (4), the most demanding specification, reinforces the formality result. Relative to informal solo workers, informal workers in firms with 2--3 workers earn 0.113 log points more, and informal workers in firms with 4--5 workers earn 0.256 log points more. The coefficients remain between 0.316 and 0.373 log points for the larger informal categories. Tables~\ref{tab:firm-size-local-geography-formality-robustness-101-reference} and \ref{tab:firm-size-local-geography-formality-robustness-11-19-reference} show the same pattern from alternative reference points: the largest informal wage gap is between solo work and very small informal units on one side and the rest of the informal firm-size distribution on the other, rather than a strictly monotonic increase through the largest category.

Among formal workers, the profile is flatter relative to formal solo workers, but formal workers in 101+ firms earn more than formal workers in the intermediate categories. In column (4), the 101+ coefficient is 0.113 log points above the 11--19 category. With 101+ workers as the reference group, the coefficients for all smaller formal categories are negative, between -0.067 and -0.225 log points.
...
\input{Paper/sections/regression_elasticity_benchmark_table}
...
Table~\ref{tab:polynomial-size-profile} and
Figure~\ref{fig:polynomial-size-profile} report the quadratic approximation in
Equation~(\ref{eq:polynomial-size-profile}). The raw polynomial profile implies
a 175.8 percent wage premium for assigned 101+ workers relative to solo workers.
After sector-year effects, worker controls, and formality are added, the
implied full-sample 101+/solo premium falls to 41.0 percent, close to the 43.2
percent estimate from the categorical specification in
Table~\ref{tab:firm-size-wage-premium-regressions}. The quadratic term is
negative and statistically significant in the full-sample specification,
consistent with a concave profile: the fitted wage premium rises with assigned
firm size but at a decreasing rate.

\input{Paper/sections/regression_polynomial_size_profile_table}

The formality split is more informative than the full-sample curve. Among
formal workers, the polynomial coefficients do not point to a statistically
significant 101+/solo premium; the implied premium is 8.6 percent and is not
significant. Among informal workers, the log-size coefficient is positive, the
quadratic term is negative, and the implied 101+/solo premium is 46.9 percent.
The polynomial approximation therefore reinforces the main formality result:
the conditional size-wage relationship is not an artifact of estimating a
separate dummy for every size bin, but the steep part of that relationship is
concentrated in informal employment. At the same time, the approximation smooths
over nonmonotonicities in the categorical estimates, so it should be read as a
shape check rather than as a replacement for the main tables.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{fig78_\figurelang.png}
